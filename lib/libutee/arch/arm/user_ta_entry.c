...
static TAILQ_HEAD(phdr_info_head, phdr_info) _phdr_info_head =
		TAILQ_HEAD_INITIALIZER(_phdr_info_head);
/*
				 ...
				 size_t size __unused, void *data)
{
	...
	qe->info = *info;
	TAILQ_INSERT_TAIL(&_phdr_info_head, qe, link);
	(*count)++;
	return 0;
}
...
void __utee_call_elf_init_fn(void)
{
	...
	TAILQ_FOREACH_REVERSE(qe, &_phdr_info_head, phdr_info_head, link) {
	...
	TAILQ_FOREACH_SAFE(qe, &_phdr_info_head, link, qe2)
		free(qe);
	TAILQ_INIT(&_phdr_info_head);
}
